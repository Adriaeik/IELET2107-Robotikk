...
1. **Feil** fordi x- og y-aksen projiserast på bildeplanet, medan z er djupna mot kameraet.  
2. **Riktig** fordi kalibrering samanliknar synlege punkt med kjent posisjon i verdsramma.  
3. **Riktig** fordi den automatiske terskelmetoden i faget bruker rekursive formlar for å berekne variansar meir effektivt når ein skal finne optimal terskelverdi.
4. **Feil** fordi du treng fleire kontrollpunkt for å bestemme alle kameraparametrane.  
5. **Riktig** fordi eit godt kalibrert kamera kan mappe frå pikselkoordinatar til verdskoordinatar.  
6. **Riktig** fordi histogram viser kor mange pikslar som har ei gitt lysstyrke.  
7. **Feil** fordi algoritmen minimerer innad-gruppe-variansen (eller maksimerer mellom-gruppe-variansen).  
8. **Riktig** fordi forsvinningspunkt er projeksjonen av sett med parallelle linjer i 3D, og dei tre forsvinningspunkta (for tre gjensidig ortogonale retningar) dannar ein trekant. Midtpunktet av denne trekanten gir pikselkoordinatane til prinsipalpunktet, som blir brukt i kamerakalibreringa.
...
2. Regn ut hva $T^{0}_{3}$, $T^{0}_{2}$ og $T^{0}_{1}$ blir. (8%)
